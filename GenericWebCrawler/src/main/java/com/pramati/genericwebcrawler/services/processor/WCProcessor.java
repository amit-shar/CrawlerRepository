...
public class WCProcessor implements Runnable {
	...
	public WCProcessor(CrawlerUtility crawlerUtilityObj,
			EmailFilterRule filterRuleObj) {
		this.filterRuleObj = new EmailFilterRule();
		this.crawlerUtilityObj = new CrawlerUtility();

	}

	public void init(BlockingQueue<String> sharedQueue, URL crawlUrl,
			String year) {
	...
	protected void crawlUtility(URL url) {
		...
		try {
			...
			String pageContent = "";
			pageContent = crawlerUtilityObj
					.convertHtmlToString(url, urlToCrawl);
			...
			if (hyperlinksSet != null) {
				...
				while (it.hasNext()) {
					...
					if (!crawledUrl.contains(baseurl.toString())
							&& filterRuleObj.filterCriteria(baseurl.toString())) {
